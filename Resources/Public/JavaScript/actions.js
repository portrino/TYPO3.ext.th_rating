...
var constDelayFlashMessage=0;
var constFlashMessageFadein=200;
var constFlashMessageDuration=1500;
var constFlashMessageFadeout=500;
...
function initBinding() {
	...
	jQuery('span.ajaxLink').one('click', function() {
		var selectedRating = jQuery('input:first',this).val();
		if (selectedRating !== undefined) {
            submitVoteForm(jQuery.parseJSON(selectedRating));
		}
		return false;
	});
...
function submitVoteForm( choosenValue ) {
		var targetObj = document.getElementById(choosenValue.ajaxRef);
...
function checkVoteSubmission(formData, jqForm, options) {
	for (var i = 0; i < formData.length; ++i) {
		...
		if (formData[i]["name"] == "tx_thrating_pi1[ajaxRef]") {
			var ajaxTargetId = formData[i]["value"];
		}
	}
	return true;
}
...
function handleReceivedVote(jsonData)  {
	var ajaxTargetId = jsonData.ajaxRef;
	var targetObj = document.getElementById(ajaxTargetId); //find target object in DOM
	var flashMessages = jsonData.flashMessages; //save flash-messages
	...
	if (jsonData.currentPollDimensions) {
		...
		jQuery.each(jsonData.currentPollDimensions, function ( step, currentPollDimension) {
			...
			jQuery('.current-poll.' + currentPollDimension.steporder + '.tx-thrating-currentPollHeight',targetObj).css('height:' + currentPollDimension.pctValue + '%; background-position:0px '+ currentPollDimension.backgroundPos + 'px');
			jQuery('.current-poll.' + currentPollDimension.steporder + '.tx-thrating-currentPollWidth',targetObj).css('width:' + currentPollDimension.pctValue + '%; background-position:0px '+ currentPollDimension.backgroundPos + 'px');
		});
	}
